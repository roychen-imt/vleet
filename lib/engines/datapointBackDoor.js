...
DatapointBackDoorEngine.prototype.sendData = async function(systemData) {
  ...
  if (this.dryRun) {
    conzole.done('Exiting due to DRY RUN...');
    return;
  }
  ...
  if (filePath) {
    ...
    conzole.failed('uploading to s3 failed');
  }
};
...
DatapointBackDoorEngine.prototype.generateMessage = function(system, dataChunk) {
  ...
  const data = _.map(dataChunk, datapoint => {
    ...
    const value = _.isNull(datapoint.value_num)
      ? datapoint.value_string
      : _.defaultTo(_.toNumber(datapoint.value_num), 0);
...
function DatapointBackDoorEngine(simulation, config) {
  ...
  this.dataGenerators = {};
  this.dryRun = _.get(simulation, 'generation.dryRun', true);
}
